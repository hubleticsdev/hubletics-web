...
type UpcomingBooking = Awaited<
  ReturnType<typeof import('@/actions/bookings/queries').getUpcomingBookings>
>['bookings'][number];

type RecurringLesson = Awaited<
  ReturnType<typeof import('@/actions/group-bookings/recurring-queries').getMyRecurringLessons>
>['lessons'][number];
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-12 px-4 pb-16 pt-10 sm:px-6 lg:px-8">
      ...
      {coach.allowPublicGroups && (
        <BookingSection
          ...
          description="Manage your recurring group lesson templates."
          badge={
            recurringLessons.length > 0
              ? `${recurringLessons.length} active series`
              : undefined
          }
          items={recurringLessons}
          empty={{
            icon: CalendarClock,
            title: 'No recurring lessons',
            description:
              'Create recurring lesson series to build consistent revenue streams.',
            action: {
              href: '/dashboard/create-group-lesson',
              label: 'Create recurring lesson'
            }
          }}
          renderItem={(lesson) => (
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <span className="ml-2 font-medium">
                          {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][lesson.dayOfWeek]}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500">Time:</span>
                        <span className="ml-2 font-medium">{lesson.startTime}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Duration:</span>
                        <span className="ml-2 font-medium">{lesson.duration} min</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Price:</span>
                        <span className="ml-2 font-medium">${lesson.pricePerPerson}</span>
                      </div>
                    </div>
                  </div>
